# ==============================================================================
# SCRIPT DE PRUEBA COMPLETA (DINAMICO) - Basado en test3.tcl
# ==============================================================================

# ------------------------------------------------------------------------------
# 0. LEER ARGUMENTOS DESDE INTERFAZ PYTHON
# ------------------------------------------------------------------------------
if { $argc < 3 } {
...
# Calcular limite de lectura para resultados (W * H * ScaleFactor^2 aprox)
# Usamos el buffer completo de entrada como seguridad
...
if { [catch {
    ...
    # CONFIGURACIÓN
    # ---------------------------------------------------------
    # Variables DINAMICAS desde Python
    set W_VAL $W_IN
    set H_VAL $H_IN
    
    puts "== 2. CARGANDO IMAGEN DE PRUEBA (${W_VAL}x${H_VAL}) =="
    for {set y 0} {$y < $H_VAL} {incr y} {
        for {set x 0} {$x < $W_VAL} {incr x} {
            set addr [expr ($y * $W_VAL) + $x]
            set val [expr (($y + 1) * 16) + $x]
            # Asegurar byte (modulo 256)
    ...
    write_reg 0 [expr $W_VAL & 0xFF]
    write_reg 1 [expr ($W_VAL >> 8) & 0xFF]
    write_reg 2 [expr $H_VAL & 0xFF]
    write_reg 3 [expr ($H_VAL >> 8) & 0xFF]
    write_reg 4 $S_HEX 
    write_reg 5 0x00
    ...
    puts "   DATA_SEQ: $seq_results"
    
    # ---------------------------------------------------------
    # LIMPIEZA
    # ---------------------------------------------------------
    puts "\n== 5. LIMPIANDO SALIDA (Borrando 0x40000...) =="
    set clear_limit [expr ($W_VAL * $H_VAL)]
    for {set k 0} {$k < $clear_limit} {incr k} {
    ...
    # LECTURA DE DATOS PARA PYTHON (CRÍTICO)
    set simd_results [list]
    puts "   Leyendo Salida SIMD..."
    
    # Leemos la cantidad estimada de salida (aprox W*H es seguro para buffer)
    for {set k 0} {$k < $OUT_LIMIT} {incr k} {
         set r [read_mem [expr 0x40000 + $k]]
         scan $r %b d; lappend simd_results $d
    }
    
    # MARCA PARA QUE PYTHON LEA LOS DATOS
    puts "DATA_SIMD: $simd_results"
